...
var BlockchainInfo = require('./blockchaininfo');
...
Address.prototype = {
  ...
  getUnspentOutputs: function() {
    ...
    this.generate().done(function() {
      // Call BlockchainInfo.
      BlockchainInfo.getUnspentOutputs(that.address).
  ...
  setUnspentOutputs: function(response) {
    ...
    this.unspentOutputs = response.unspent_outputs.map(function(output) {
      return {
        hash: output.tx_hash_big_endian, // could also be tx_hash?
        index: output.tx_index,
        amount: output.value,
